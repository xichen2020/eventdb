...
// doubleResultGroupHeap is a heap storing a list of values.
// The ordering of such items are determined by `lessThanFn`.
// The smallest item will be at the top of the heap.
type doubleResultGroupHeap struct {
	dv         []doubleResultGroup
	lessThanFn func(v1, v2 doubleResultGroup) bool
}

// newDoubleResultGroupHeap creates a new values heap.
func newDoubleResultGroupHeap(
	initCapacity int,
	lessThanFn func(v1, v2 doubleResultGroup) bool,
) *doubleResultGroupHeap {
	return &doubleResultGroupHeap{
		dv:         make([]doubleResultGroup, 0, initCapacity),
		lessThanFn: lessThanFn,
	}
}

// RawData returns the underlying backing array in no particular order.
func (h doubleResultGroupHeap) RawData() []doubleResultGroup { return h.dv }

// Min returns the "smallest" heap element according to the `lessThan` function.
func (h doubleResultGroupHeap) Min() doubleResultGroup { return h.dv[0] }

// Len returns the number of items in the heap.
func (h doubleResultGroupHeap) Len() int { return len(h.dv) }

// Cap returns the heap capacity before a reallocation is needed.
func (h doubleResultGroupHeap) Cap() int { return cap(h.dv) }

// Less returns true if item `i` is less than item `j`.
func (h doubleResultGroupHeap) Less(i, j int) bool {
	return h.lessThanFn(h.dv[i], h.dv[j])
}

// Swap swaps item `i` with item `j`.
func (h doubleResultGroupHeap) Swap(i, j int) { h.dv[i], h.dv[j] = h.dv[j], h.dv[i] }

// Reset resets the internal backing array.
func (h *doubleResultGroupHeap) Reset() { h.dv = h.dv[:0] }

// Push pushes a value onto the heap.
func (h *doubleResultGroupHeap) Push(value doubleResultGroup) {
	h.dv = append(h.dv, value)
	h.shiftUp(h.Len() - 1)
}

// Pop pops a value from the heap.
func (h *doubleResultGroupHeap) Pop() doubleResultGroup {
	var (
		n   = h.Len()
		val = h.dv[0]
	)
...
func (h *doubleResultGroupHeap) SortInPlace() []doubleResultGroup {
	numElems := len(h.dv)
	for len(h.dv) > 0 {
		h.Pop()
	}
...
func (h doubleResultGroupHeap) shiftUp(i int) {
	for {
		parent := (i - 1) / 2
...
func (h doubleResultGroupHeap) heapify(i, n int) {
	for {
		left := i*2 + 1
		right := left + 1
		smallest := i

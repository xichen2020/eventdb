...
// *BucketizedStringArrayPool is a generic bucketized value array pool.
...
func (rv *RefCountedPooledStringArray) Get() []string { return rv.vals }
...
// to the snapshot.
func (rv *RefCountedPooledStringArray) Snapshot() *RefCountedPooledStringArray {
	rv.cnt.IncRef()
	return &RefCountedPooledStringArray{
		cnt:  rv.cnt,
		p:    rv.p,
...
func (rv *RefCountedPooledStringArray) Append(v string) {
...
func (rv *RefCountedPooledStringArray) Close() {
	if rv.closed {
		return
	}
	rv.closed = true
...
func (rv *RefCountedPooledStringArray) tryRelease() {
	if rv.cnt.DecRef() > 0 {
		return
	}
